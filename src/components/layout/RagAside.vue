...
const menus = computed(() => {
  const ragRoute = router.options.routes.find(route => route.path === '/rag')
  return ragRoute?.children || []
...
const getFullPath = (subItem: any) => {
  // If the subItem path already starts with '/', it's already absolute
  if (subItem.path.startsWith('/')) {
    return subItem.path
  }
  // Otherwise, prepend the parent path '/rag'
  return `/rag/${subItem.path}`
}
</script>
  ...
  <el-scrollbar>
    <el-menu :default-active="route.path" :collapse="collapse" router>
      <template v-for="subItem in menus" :key="subItem.path">
        ...
        <el-menu-item v-else :index="getFullPath(subItem)">
          <el-icon>
            <component :is="subItem.meta?.icon"></component>
          </el-icon>
          <template #title>
