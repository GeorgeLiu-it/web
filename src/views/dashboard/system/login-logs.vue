<template>
  <div class="login-logs">
    <div class="title">
      <el-row>Login Traces</el-row>
    </div>

    <div class="user-login-list-request">
      <el-form :inline="true" :model="userLoginListRequest">
        <el-form-item label="uuid">
          <el-input v-model="userLoginListRequest.uuid" placeholder="Pleas input UUID" clearable/>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" icon="Search" @click="getUserLoginTableData">Search</el-button>
        </el-form-item>
      </el-form>
    </div>
    ...
    >
      ...
      <el-table-column label="Login time">
        <template #default="scope:{ row: Login, column: any, $index: number }">
          {{ getTime(scope.row.created_at) }}
        </template>
      </el-table-column>
      <el-table-column prop="login_method" label="Login method"/>
      <el-table-column prop="ip" label="IP"/>
      <el-table-column prop="address" label="Addr"/>
      <el-table-column prop="os" label="OS"/>
      <el-table-column prop="device_info" label="Device"/>
      <el-table-column prop="browser_info" label="Browser"/>
